...
defaults('USE_AUTHENTICATION',0);			// Use (internal) authentication - best choice if
											...
											//  this work!
defaults('ADMIN_USERNAME','timo'); 			// Admin Username
defaults('ADMIN_PASSWORD','124');  	// Admin Password - CHANGE THIS TO ENABLE!!!
...
$PHP_SELF = './mod.php?M=Apc_AoeApcApc';
...
function menu_entry($ob,$title) {
	...
	if ($MYREQUEST['OB']!=$ob) {
		return "<li><a href=\"./mod.php?M=Apc_AoeApcApc&OB=$ob\">$title</a></li>";
	} else if (empty($MYREQUEST['SH'])) {
		return "<li><span class=active>$title</span></li>";
	} else {
		return "<li><a class=\"child_active\" href=\"./mod.php?M=Apc_AoeApcApc&OB=$ob\">$title</a></li>";
	}
}
